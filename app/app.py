...
def create_app():
    ...
    @flask_app.route("/")
    def index():
        # Use the Flask app's configured static folder to serve the
        # frontend's index.html reliably. Calling send_from_directory
        # with a plain "static" string may resolve relative to the
        # current working directory and fail when the app is started
        # from a different path. `send_static_file` uses the
        # `static_folder` configured on the Flask app.
